...
class FactionRank{
	...
	public static function getDefaultRanks(PocketFactions $plugin){
		return $plugin->getOpts()->defaultRanks;
	}
	public static function fromConfig(\Logger $logger, $id, $rank){
		$bitmask0 = 0;
		$bitmask1 = 0;
		foreach($rank["permissions"] as $perm){
		...
		foreach($rank["excluded permissions"] as $perm){
			$bit = FactionPermission::fromString($perm, $tier);
			if($tier === -1){
				$logger->warning("Unknown permission \"$perm\" will be ignored.");
				continue;
		...
		return new self($id, $rank["name"], $bitmask0, $bitmask1);
	}
	/**
	 * @return int
	 */
